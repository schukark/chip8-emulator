    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            ...
            match self {
                ...
                Instruction::EqConst { x, value } => format!("SE V{x:#01X}, {value:#02X}"),
                Instruction::NeqConst { x, value } => format!("SNE V{x:#01X},{value:#02X}"),
                Instruction::EqReg { x, y } => format!("SE V{x:#01X}, V{y:#01X}"),
                Instruction::AssignMem { x, value } => format!("LD V{x:#01X}, {value:#02X}"),
                Instruction::AddAssignMem { x, value } => format!("ADD V{x:#01X}, {value:#02X}"),
                Instruction::AssignReg { x, y } => format!("LD V{x:#01X}, {y:#01X}"),
                Instruction::OrReg { x, y } => format!("OR V{x:#01X}, {y:#01X}"),
                Instruction::AndReg { x, y } => format!("AND V{x:#01X}, {y:#01X}"),
                Instruction::XorReg { x, y } => format!("XOR V{x:#01X}, {y:#01X}"),
                Instruction::AddAssignReg { x, y } => format!("ADD V{x:#01X}, {y:#01X}"),
                Instruction::SubAssignReg { x, y } => format!("SUB V{x:#01X}, {y:#01X}"),
                Instruction::RShift { x, y: _y } => format!("SHR V{x:#01X}"),
                Instruction::SubAssignRegInverse { x, y } => format!("SUBN V{x:#01X}, {y:#01X}"),
                Instruction::LShift { x, y: _y } => format!("SHL V{x:#01X}"),
                Instruction::NeqReg { x, y } => format!("SNE V{x:#01X}, {y:#01X}"),
                Instruction::SetI { address } => format!("LD I, {address:#03X}"),
                Instruction::GotoPlusV0 { address } => format!("JP V0, {address:#03X}"),
                Instruction::Rand { x, value } => format!("RND V{x:#01X}, {value:#02X}"),
                Instruction::DrawSprite { x, y, height } =>
                    format!("DRW V{x:#01X}, V{y:#01X}, {height}"),
                Instruction::KeyPressedSkip { x } => format!("SKP V{x:#01X}"),
                Instruction::KeyReleasedSkip { x } => format!("SKNP V{x:#01X}"),
                Instruction::GetDelayTimer { x } => format!("LD V{x:#01X}, DT"),
                Instruction::AwaitKeyPress { x } => format!("LD V{x:#01X}, K"),
                Instruction::SetDelayTimer { x } => format!("LD DT, V{x:#01X}"),
                Instruction::SetSoundTimer { x } => format!("LD ST, V{x:#01X}"),
                Instruction::AddAssignAddress { x } => format!("ADD I, V{x:#01X}"),
                Instruction::SetSpriteAddr { x } => format!("LD F, V{x:#01X}"),
                Instruction::SetBCD { x } => format!("LD B, V{x:#01X}"),
                Instruction::DumpRegisters { x } => format!("LD [I], V{x:#01X}"),
                Instruction::LoadRegisters { x } => format!("LD V{x:#01X}, [I]"),
            }
        )
    }
}
